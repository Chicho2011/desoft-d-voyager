...
class VoyagerBaseController extends BaseController
{
    ...
    public function index(Request $request)
    {
        ...
        $onlyOneRecord = app($dataType->model_name)->maxRecords == 1;
        ...
        $view = view()->exists('dvoyager::bread.browse') ? 'dvoyager::bread.browse' : 'voyager::bread.browse';
    ...
    public function create(Request $request)
    {
        ...
        if(isset($dataTypeContent->maxRecords))
        {
            if($dataTypeContent->maxRecords > 0)
            {
                $total = count($dataTypeContent::all());
                if($total >= $dataTypeContent->maxRecords)
                {
                    return back()->with([
                        'message' => __('dvoyager::validation.records_limit', ['max' => $dataTypeContent->maxRecords]),
                        'alert-type' => 'error'
                    ]);
                }
            }
